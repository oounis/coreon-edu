...
export default function Security() {
  ...
  return (<>
    <PageHead title={t('Poste de sécurité')} sub={`${settings().schoolName} ${t('· portail, visiteurs, rondes et soirées')}`}
      action={<Tabs value={tab} onChange={setTab} tabs={[
        { value: 'evenements', label: t('À couvrir'), count: toCover.length || undefined },
        { value: 'visiteurs', label: t('Visiteurs'), count: insideNow || undefined },
        { value: 'rondes', label: t('Rondes') },
        { value: 'journal', label: t('Main courante') },
        { value: 'consignes', label: t('Consignes') },
      ]} />} />

    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
      <StatCard tint="brand" icon={<CalendarClock size={20} />} value={tonight.length} label={t('Ce soir')} sub={tonight.length ? t('à couvrir') : t('rien de prévu')} onClick={() => setTile('tonight')} />
      <StatCard tint="grape" icon={<UserCheck size={20} />} value={insideNow} label={t("Visiteurs dans l'école")} onClick={() => setTile('inside')} />
      <StatCard tint="sky" icon={<Flashlight size={20} />} value={(d.rounds || []).filter(r => r.date === today).length} label={t("Rondes aujourd'hui")} onClick={() => setTile('rounds')} />
      <StatCard tint="coral" icon={<AlertTriangle size={20} />} value={openIncidents} label={t('Incidents ouverts')} onClick={() => setTile('incidents')} />
    </div>
          ...
          : tonight.map(ev => (
            <div key={ev.id} className="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-canvas">
              <span className="min-w-0 flex-1">
                ...
                <span className="block text-[12px] text-muted">{ev.time} · {ev.place} · {t('organisé par')} {ev.byName}</span></span>
              <Btn size="sm" variant="soft" onClick={() => { setTile(null); setTab('evenements') }}>{t('Check-list')}</Btn>
            </div>)) },
        inside: { title: `${t("Visiteurs dans l'école")} · ${insideList.length}`, body: insideList.length === 0
          ? <EmptyState icon={<DoorOpen size={24} />} title={t("Aucun visiteur dans l'école")} sub={t('Toutes les visites du jour sont terminées.')} />
          : insideList.map(v => (
            <div key={v.id} className="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-canvas">
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="block text-[12px] text-muted truncate">{v.purpose} · {t('reçu par')} {v.hostName} · {t('entré à')} {v.inAt}</span></span>
            </div>)) },
        rounds: { title: `${t("Rondes aujourd'hui")} · ${roundsList.length}`, body: roundsList.length === 0
          ? <EmptyState icon={<Flashlight size={24} />} title={t("Aucune ronde aujourd'hui")} sub={t("Démarrez une ronde depuis l'onglet Rondes.")} />
        ...
        return (
          <Card key={ev.id} className="p-5 flex flex-col gap-3">
            ...
            {notice.short && <div className="flex items-start gap-2 text-[12px] rounded-xl px-3 py-2" style={{ background: STATUS.warnSoft, color: '#8A5A12' }}>
              ...
              <span>{t('Préavis court :')} <b>{notice.hours} h</b> {t("avant l'événement (minimum recommandé :")} {SECURITY_NOTICE_H} {t('h). Signalez-le à la Direction.')}</span>
            </div>}

            <div>
              <div className="text-[12px] font-bold text-muted mb-1">{t('Pourquoi une présence est nécessaire')}</div>
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-[12px] text-muted">
              <span className="inline-flex items-center gap-1"><Users size={12} />{adultCount(ev)} {t('adultes')}{childCount(ev) > 0 && ` · ${childCount(ev)} ${t('enfants')}`}</span>
              <span className="inline-flex items-center gap-1"><UserCheck size={12} />{t('organisateur :')} {ev.byName}</span>
            </div>

            <div>
              <div className="flex items-center justify-between text-[12px] mb-1">
                <span className="font-bold">{t('Check-list')}</span>
            ...
            <Btn size="sm" onClick={() => setOpen(ev)} className="mt-auto">
              <ShieldCheck size={15} /> {done ? t('Poursuivre la check-list') : t('Ouvrir la check-list')}</Btn>
          </Card>)
      })}
    </div>
...
function ChecklistModal({ ev, u, onClose, refresh }) {
  ...
  const finish = () => {
    ...
    toast.success(t('Check-list close · Direction prévenue · main courante mise à jour'))
    onClose()
  }

  return (
    <Modal open onClose={onClose} title={ev.title} size="2xl"
      footer={<><Btn variant="ghost" onClick={onClose}>{t('Fermer')}</Btn>
...
function VisitorsTab({ u, d, refresh }) {
  ...
  return (<>
    <SectionCard icon={<DoorOpen size={16} />} tint="grape" title={t('Registre des visiteurs')} sub={`${today} · ${inside.length} ${t("personne(s) actuellement dans l'école")}`}
      action={<Btn size="sm" onClick={() => { setF(BLANK_V()); setOpen(true) }}><Plus size={14} /> {t('Enregistrer une entrée')}</Btn>} bodyClass="p-3">
      {list.length === 0
        ? <EmptyState icon={<DoorOpen size={24} />} title={t("Aucun visiteur aujourd'hui")} sub={t("Chaque personne extérieure est enregistrée avec sa pièce d'identité et un badge.")} />
            ...
            {list.map(v => (
              <div key={v.id} className="flex items-center gap-3 p-2.5 rounded-xl border border-line">
                ...
                <div className="text-right shrink-0">
                  ...
                  {isInside(v)
                    ? <Btn size="sm" variant="soft" onClick={() => checkOut(v)}><LogOut size={13} /> {t('Sortie')}</Btn>
                    : <span className="text-[11px] font-bold px-2 py-0.5 rounded-full" style={{ background: STATUS.okSoft, color: STATUS.ok }}>{t('sorti')}</span>}
                </div>
              </div>))}
          </div>}
    </SectionCard>
    ...
    <Modal open={open} onClose={() => setOpen(false)} title={t("Entrée d'un visiteur")} size="xl"
      ...
      {needsEscort(f.purpose) && <div className="mt-3 text-[12px] rounded-xl px-3 py-2.5 flex items-start gap-2" style={{ background: STATUS.warnSoft, color: '#8A5A12' }}>
        <AlertTriangle size={14} className="mt-0.5 shrink-0" />
        <span>{t('Un visiteur qui approche les élèves')} <b>{t('ne circule jamais seul')}</b>{t('. Badge remis, accompagnateur désigné, sortie enregistrée.')}</span>
      </div>}
    </Modal>
  </>)
}
...
function RoundsTab({ u, d, refresh }) {
  ...
  const finish = () => {
    ...
    if (!points.length) return toast.error(t('Pointez au moins une zone'))
    ...
    mutate(db => {
      ...
      db.logbook.unshift({ id: uid('l'), at: Date.now(), agentName: u.name, kind: 'ronde', place: t('Périmètre'),
        text: `${t('Ronde effectuée ·')} ${points.length} ${t('point(s) de passage')}${anomalies.length ? ` · ${anomalies.length} ${t('anomalie(s)')}` : t(' · rien à signaler')}.` })
    ...
    if (anomalies.length) notify({ role: 'schooladmin', kind: 'info', actor: u.name, title: t('Anomalies relevées en ronde'),
      body: anomalies.map(a => `${checkpointOf(a.k).label} : ${a.anomaly}`).join(' · '), link: '/app/security' })
    toast.success(`${t('Ronde close ·')} ${points.length} ${t('zones')}${anomalies.length ? ` · ${anomalies.length} ${t('anomalie(s) signalée(s)')}` : ''}`)
  ...
  return (<>
    ...
    <SectionCard icon={<Clock size={16} />} tint="slate" title={t('Rondes du jour')} bodyClass="p-3">
      {rounds.length === 0 ? <EmptyState icon={<Flashlight size={24} />} title={t("Aucune ronde aujourd'hui")} sub={t('Démarrez une ronde ci-dessus.')} />
        : rounds.map(r => (
...
function LogTab({ u, d, refresh }) {
  const [open, setOpen] = useState(false)
  const [f, setF] = useState({ kind: 'consigne', place: t('Poste de garde'), text: '' })
  const log = [...(d.logbook || [])].sort((a, b) => b.at - a.at)

  const add = () => {
    if (!f.text.trim()) return toast.error(t('Écrivez ce que vous constatez'))
    ...
    toast.success(t('Inscrit à la main courante')); setOpen(false); setF({ kind: 'consigne', place: t('Poste de garde'), text: '' }); refresh()
  }
  const shift = kind => {
    mutate(db => { db.logbook.unshift({ id: uid('l'), at: Date.now(), agentName: u.name, kind, place: t('Poste de garde'),
      text: kind === 'prise' ? t('Prise de service.') : t('Fin de service. Clés rendues.') }) })
    toast.success(kind === 'prise' ? t('Prise de service enregistrée') : t('Fin de service enregistrée')); refresh()
  }

  return (<>
    <SectionCard icon={<BookOpen size={16} />} tint="slate" title={t('Main courante')}
      sub={t("Le journal du poste : prise de service, rondes, visiteurs, anomalies. On y écrit, on n'y efface pas.")}
      action={<div className="flex gap-2">
        <Btn size="sm" variant="soft" onClick={() => shift('prise')}><LogIn size={13} /> {t('Prise de service')}</Btn>
        <Btn size="sm" variant="soft" onClick={() => shift('fin')}><LogOut size={13} /> {t('Fin de service')}</Btn>
        <Btn size="sm" onClick={() => setOpen(true)}><Plus size={14} /> {t('Écrire')}</Btn>
      </div>} bodyClass="p-3">
      {log.length === 0 ? <EmptyState icon={<BookOpen size={24} />} title={t('Main courante vide')} sub={t('Commencez par votre prise de service.')} />
    ...
    <Modal open={open} onClose={() => setOpen(false)} title={t('Écrire à la main courante')}
      footer={<><Btn variant="ghost" onClick={() => setOpen(false)}>{t('Annuler')}</Btn><Btn onClick={add}>{t('Inscrire')}</Btn></>}>
      <div className="grid sm:grid-cols-2 gap-3">
        <Field label={t('Nature')}><Select value={f.kind} onChange={e => setF({ ...f, kind: e.target.value })}>
          {Object.entries(LOG_KINDS).filter(([k]) => !['prise', 'fin'].includes(k)).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
        </Select></Field>
        <Field label={t('Lieu')}><Input value={f.place} onChange={e => setF({ ...f, place: e.target.value })} /></Field>
        <div className="sm:col-span-2"><Field label={t('Constat')} hint={t('Faits, heure, personnes, actions menées.')}>
          <Textarea rows={3} value={f.text} onChange={e => setF({ ...f, text: e.target.value })} placeholder={t('Ce que vous avez vu, fait, et ce qui a suivi.')} /></Field></div>
      </div>
      <p className="text-[12px] text-muted mt-2">{t('Une')} <b>{t('anomalie')}</b> {t('ou une')} <b>{t('alarme')}</b> {t('prévient immédiatement la Direction.')}</p>
    </Modal>
  </>)
}
