// ════════════════════════════════════════════════════════════════════════════
// SITE PUBLIC — briques partagées entre les pages de la vitrine.
//
// CR-006 : la navigation n'est plus un défilement dans une seule page. Chaque
// entrée du menu (Accueil, Modules, Vos données, Tarifs, FAQ) est une VRAIE
// route avec sa propre URL, que l'on peut partager, mettre en favori, et que le
// bouton « précédent » du navigateur respecte. Les sections vivaient toutes dans
// Landing.jsx ; elles deviennent ici des composants réutilisables, composés par
// des pages routées (voir site/Home.jsx, site/ModulesPage.jsx, etc.).
// ════════════════════════════════════════════════════════════════════════════
...
import {
  ArrowRight, ClipboardCheck, Wallet, ShieldAlert, MessageSquare, Check,
...
export const up = { initial:{opacity:0,y:20}, whileInView:{opacity:1,y:0}, viewport:{once:true,margin:'-60px'}, transition:{duration:.5} }
const BTN="inline-flex items-center justify-center gap-1.5 rounded-xl font-semibold transition active:scale-[.98]"
export const BTN_MD=`${BTN} text-sm px-4 py-2.5`, BTN_LG=`${BTN} text-sm px-5 py-3`
export const A='#7539E4'   // le violet Coreon Edu — KOGIA_HARMONY §4.2

// Les entrées du menu = les routes de la vitrine. L'ordre est celui du parcours.
export const NAV = [
  ['/', 'Accueil'],
  ['/modules', 'Modules'],
  ['/donnees', 'Vos données'],
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <motion.div {...up} className="card p-7">
            ...
            <ul className="mt-4 space-y-2.5">
              {[t('Journal du jour : repas, sieste, change, humeur'),t('Personnes autorisées à récupérer l’enfant'),t('Carnet de vaccination et jalons de développement'),t('Déclaration d’accident avec accusé du parent')].map(x=>(
                <li key={x} className="flex items-start gap-2.5 text-sm"><Check size={15} className="mt-0.5 shrink-0" style={{color:'#BE185D'}}/>{x}</li>))}
            </ul>
          </motion.div>
          <motion.div {...up} className="card p-7">
            ...
            <ul className="mt-4 space-y-2.5">
              {[t('Évaluation quotidienne partagée en direct'),t('Présence, retards, alertes aux parents'),t('Bulletins, passage d’année, archives'),t('Emploi du temps et suivi par matière')].map(x=>(
                <li key={x} className="flex items-start gap-2.5 text-sm"><Check size={15} className="mt-0.5 shrink-0" style={{color:A}}/>{x}</li>))}
            </ul>
          </motion.div>
        </div>
      ...
      <motion.div {...up} className="grid lg:grid-cols-2 gap-12 items-center">
        ...
        <div className="order-1 lg:order-2">
          ...
          <ul className="mt-5 space-y-3">
            {[t('Glisser-déposer : Excellent · Bien · Moyen · Insuffisant'),t('Badges de motivation, jamais de classement public'),t('Les parents suivent la progression leçon par leçon')].map(x=>(
              <li key={x} className="flex items-start gap-3"><span className="w-6 h-6 rounded-full grid place-items-center shrink-0 mt-0.5" style={{background:'#EEF2FF',color:A}}><Check size={14}/></span><span className="text-sm">{x}</span></li>
            ))}
          </ul>
        </div>
      </motion.div>
      ...
      <div className="mx-auto max-w-[1120px] px-5">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {[[HeartPulse,t('1 · Le témoin déclare'),t('L’adulte qui a vu pointe la blessure sur un schéma corporel et raconte. Il n’invente pas de jargon : il montre.')],
            ...
            [Check,t('4 · Le parent signe'),t('L’accusé de réception du parent clôt la chaîne. Sans signature, l’école voit qui relancer. Rien ne s’efface, jamais.')]].map(([Icon,titre,d],i)=>(
            <motion.div key={titre} {...up} transition={{duration:.4,delay:i*.06}} className="card p-6">
              ...
              <h3 className="font-bold mt-4 text-[15px]">{titre}</h3>
...
export function ModulesGrid(){
  return (
    <section id="modules" className="mx-auto max-w-[1120px] px-5 py-16">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {MODULES.map(([Icon,titre,d,c],i)=>(
          <motion.div key={titre} {...up} transition={{duration:.4,delay:i*.03}} className="card p-6 hover:shadow-xl transition">
            <span className="w-12 h-12 rounded-2xl grid place-items-center" style={{background:c+'1A',color:c}}><Icon size={22}/></span>
            <h3 className="font-bold mt-4">{titre}</h3>
            <p className="text-sm text-muted mt-1.5">{d}</p>
          </motion.div>
        ))}
      </div>
  ...
  return (
    <section className="mx-auto max-w-[1120px] px-5 py-16">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {[[BookUser,t('Un enfant ne part jamais avec quelqu’un qui n’est pas sur la liste.')],
          [Eye,t('Deux paires d’yeux sur chaque déclaration d’accident : qui déclare ne valide pas.')],
          [Users,t('Personne ne valide sa propre demande : congé ou dépense.')],
          [Landmark,t('Une facture émise ne se modifie pas : on l’annule par un avoir daté et motivé.')],
          [ShieldCheck,t('On archive, on ne supprime jamais un dossier scolaire.')],
          [Star,t('On observe un enfant, on ne le note pas : et on ne le compare à personne.')]].map(([Icon,titre],i)=>(
          <motion.div key={titre} {...up} transition={{duration:.4,delay:i*.04}} className="card p-6 flex items-start gap-3">
...
export function Portals(){
  return (
    <section id="portails" className="bg-canvas border-y border-line py-16">
      <div className="mx-auto max-w-[1120px] px-5">
        <motion.div {...up} className="text-center max-w-[60ch] mx-auto mb-10">
          <div className="text-xs font-bold uppercase tracking-widest" style={{color:A}}>{t('Un portail par rôle')}</div>
          <h2 className="text-3xl md:text-4xl font-extrabold mt-3">{t('Chacun voit ce qui le concerne')}</h2>
          <p className="text-muted mt-3">{t('Accès refusé par défaut, ouvert par rôle. Un parent ne voit jamais les finances ; un enseignant, jamais un autre établissement.')}</p>
        </motion.div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
  ...
  return (
    <div className="card p-4 shadow-2xl" style={{boxShadow:'0 30px 60px -25px rgba(117,57,228,.45)'}}>
      ...
      <div className="flex items-center justify-between mt-3 pt-3 border-t border-line">
        ...
        <button className="text-xs font-bold text-white px-4 py-2 rounded-lg" style={{background:A}}>{t('Enregistrer & partager')}</button>
      </div>
    </div>
  )
}
