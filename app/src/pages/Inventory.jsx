        ...
        <SectionCard key={cat.key} icon={<Boxes size={16} />} tint="sky" title={cat.label} bodyClass="p-3" className="mb-4">
          {list.map(it => (
            <div key={it.id} className="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-canvas">
              <button onClick={() => setHist(it.id)} title={t('Historique des mouvements')} className="min-w-0 flex-1 text-left k-press">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <Btn size="sm" variant="ghost" onClick={() => move(it.id, +1)} aria-label={t('Entrer un')}><Plus size={14} /></Btn>
              </div>
            </div>))}
        </SectionCard>))}
