...
const when = v => new Date(v).toLocaleString(dateLocale(), { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })
...
export default function Accidents() {
  ...
  if (isParent) {
    ...
    if (!mine.length) return <EmptyState icon="ShieldCheck" title={t('Aucune déclaration.')}
      sub={t('S’il arrive quelque chose à votre enfant, l’école vous le dira ici : et vous devrez le confirmer.')} />
    return (
      <>
        <PageHead title={t('Déclarations d’accident')} sub={t('Ce que l’école vous a signalé. Merci de confirmer que vous l’avez lu.')} />
            ...
            <Card key={a.id} className="p-5">
              ...
              {!!a.care?.length && (
                <p className="text-[13px] text-muted mt-2">
                  <b>{t('Soins donnés :')}</b> {a.care.map(c => CARE[c]).join(' · ')}
                </p>
              )}
              <div className="text-[12px] text-muted mt-3">
                {t('Constaté par')} {a.witness.name} {t('· validé par')} {a.approver?.name}
              </div>

              {a.stage === 'envoye' ? (
                <div className="mt-4 rounded-xl p-4" style={{ background: STATUS.warnSoft }}>
                  <p className="text-[13px] mb-3">
                    <b>{t('Merci de confirmer que vous avez lu cette déclaration.')}</b> {t('C’est ce qui permet à l’école de savoir que vous êtes au courant.')}
                  </p>
                  <Btn onClick={async () => {
                    ...
                    <Ic n="Check" size={15} /> {t('J’ai lu et je confirme')}
                  </Btn>
                </div>
              ) : (
                <div className="mt-4 text-[13px] font-bold flex items-center gap-1.5" style={{ color: STATUS.ok }}>
                  <Ic n="CheckCheck" size={15} /> {t('Confirmé le')} {when(a.ack.at)}
                </div>
              )}
            </Card>
          ))}
  ...
  return (
    <>
      <PageHead title={t('Déclarations d’accident')} sub={t('Deux paires d’yeux, un accusé du parent, et rien qui s’efface.')}
        action={<Btn onClick={() => setOpen(true)}><Ic n="Plus" size={15} /> {t('Déclarer')}</Btn>} />
        ...
        <Card className="p-5 mb-4" style={{ background: STATUS.warnSoft }}>
          <div className="flex items-start gap-3">
            ...
            <div className="text-[13px] w-full">
              <b>{waiting.length} {t('parent(s) n’ont pas encore confirmé avoir lu.')}</b>
      ...
      {!all.length && <EmptyState icon="ShieldCheck" title={t('Aucune déclaration.')}
        sub={t('Quand un enfant se blesse, tout se joue sur la trace qu’on en garde.')} />}
          ...
          return (
            <Card key={a.id} className="p-5">
              ...
              <div className="text-[12px] text-muted mt-3 grid gap-0.5">
                <span>{t('Constaté par')} <b>{a.witness.name}</b> {when(a.witness.at)}</span>
                {a.approver && <span>{t('Validé par')} <b>{a.approver.name}</b> {when(a.approver.at)}</span>}
                {a.sentAt && <span>{t('Envoyé au parent ·')} {when(a.sentAt)}</span>}
              ...
              <div className="flex items-center gap-2 flex-wrap mt-4">
                {a.stage === 'brouillon' && (
                  mineDeclared
                    ? <span className="text-xs text-muted font-semibold flex items-center gap-1">
                        <Ic n="Lock" size={12} /> {t('Vous l’avez rédigée : un autre responsable doit la valider.')}
                      </span>
                    : <Btn size="sm" onClick={() => {
                ...
                <input value={note[a.id] || ''} onChange={e => setNote({ ...note, [a.id]: e.target.value })}
                  placeholder={t('Ajouter une note (rien ne s’efface)…')}
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div>
          ...
          <p className="text-[11px] text-muted text-center mt-2">
            {t('Touchez les zones. On ne remplit pas un menu déroulant quand on tient un enfant qui pleure.')}
          </p>
        </div>
        <div className="grid gap-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label={t('Gravité')}>
              <Select value={severity} onChange={e => setSeverity(e.target.value)}>
                {Object.values(SEVERITY).map(s => <option key={s.key} value={s.key}>{s.label}</option>)}
              </Select>
            </Field>
          </div>
          <Field label={t('Ce qui s’est passé *')} hint={t('C’est ce que le parent lira. Écrivez-le comme vous le lui diriez.')}>
            <textarea rows={3} value={what} onChange={e => setWhat(e.target.value)}
              placeholder={t('Adam a glissé en courant dans la cour et s’est cogné le genou contre le banc.')}
              className="w-full rounded-xl border border-line px-3 py-2 text-sm accent-ring" />
          </Field>
          <div>
            <div className="text-xs font-semibold text-muted mb-1.5">{t('Soins donnés')}</div>
