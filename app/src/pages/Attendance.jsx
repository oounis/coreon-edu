...
function StudentsInsights(){
  ...
  const A=useMemo(()=>{
    ...
    const trend=dates.slice(-20).map(iso=>{ const x=days[iso]; const total2=x.present+x.absent+x.late
      return {name:format(new Date(iso),'d MMM',{locale: df()}), taux:total2?Math.round(x.present/total2*100):100} })
  ...
  if(!A.latest) return <Card><EmptyState icon={<CalendarCheck size={26}/>} title={t('Aucun appel enregistré')} sub={t('Les appels des enseignants alimenteront cette vue.')}/></Card>
  ...
  return (<>
    ...
    <div className="grid lg:grid-cols-[1fr_360px] gap-4 mb-4">
      <SectionCard icon={<TrendingUp size={16}/>} tint="mint" title={t("Taux de présence de l'école")} sub={t("20 derniers jours d'école")}>
    ...
    <div className="grid lg:grid-cols-2 gap-4">
      <SectionCard icon={<AlertTriangle size={16}/>} tint="butter" title={t('Absences répétées · 30 derniers jours')} sub={t('Élèves à suivre de près (4 absences ou plus)')} bodyClass="p-3">
        {A.chronic.length===0 ? <EmptyState icon={<Check size={22}/>} title={t('Aucun absentéisme répété')} sub={t("Aucun élève n'a manqué 4 jours ou plus ce mois-ci.")}/>
        : A.chronic.map(x=>(
...
function StatDetailModal({ detail, onClose, A, today, rate, dayLabel, dayPerClass, notifyParent }){
  ...
  const Row=({s,classId,status,extra,body})=>(
    <div className="flex items-center gap-2.5 px-2 py-2 rounded-xl hover:bg-canvas">
      ...
      <button onClick={()=>notifyParent(s,body)} title={t('Prévenir le parent')}
  ...
  const C={
    rate:{ title:`Taux de présence · ${dayLabel}`, body:(<>
      ...
      <div className="space-y-2.5">
        {dayPerClass.map(([cid,c])=>{ const total=c.present+c.absent+c.late; const r=total?Math.round(c.present/total*100):100
          const col=r>=95?STATUS.ok:r>=90?STATUS.warn:STATUS.danger
          return (
            <div key={cid} className="flex items-center gap-3">
...
function MarkView(){
  ...
  if(isSummer()) return (<>
    <PageHead title={t('Appel / Présence')} sub={t("Pas d'appel pendant les vacances d'été.")}/>
  ...
  return (<>
    ...
    </div><p className="text-xs text-muted mt-2 px-1">{t("Touchez un élève pour changer : présent → absent → retard. La direction et les parents concernés sont notifiés à l'enregistrement.")}</p></Card>
