...
export default function Exams(){
  ...
  const add=()=>{ if(!f.date){toast.error('Choisissez une date');return}
    ...
    notify({role:'parent',classId:examClassId,email:true,kind:'notice',title:t('Nouvel examen programmé'),body:`${f.subject} · ${f.class} le ${f.date}`,link:'/app/exams'})
    notify({role:'teacher',kind:'info',title:t('Examen ajouté au calendrier'),body:`${f.subject} · ${f.class} le ${f.date}`,link:'/app/exams'})
  ...
  return (<>
    <PageHead title={t('Examens')} sub={t('Calendrier des prochains examens')} action={canEdit&&<Btn onClick={()=>setOpen(true)}><Plus size={16}/> {t('Programmer un examen')}</Btn>}/>
    ...
    <Modal open={open} onClose={()=>setOpen(false)} title={t('Programmer un examen')} footer={<><Btn variant="ghost" onClick={()=>setOpen(false)}>{t('Annuler')}</Btn><Btn onClick={add}>{t('Programmer')}</Btn></>}>
      <div className="grid sm:grid-cols-2 gap-3">
        <Field label={t('Classe')}><Select value={f.class} onChange={e=>setF({...f,class:e.target.value})}>{d.classes.map(c=><option key={c.id}>{c.name}</option>)}</Select></Field>
