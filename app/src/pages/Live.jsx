  ...
  return (<>
    ...
    <div className="grid lg:grid-cols-[1fr_340px] gap-5">
      ...
      <div className="space-y-5">
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between mb-1"><div className="font-bold text-sm">{phase==='after'?t('Revoir la journée'):phase==='live'?'Explorer la journée':'Explorer une journée type'}</div>
            <button onClick={()=>{setMin(defMin);setLiveNow(phase==='live')}} className="text-xs font-semibold accent-text">{phase==='live'?t('Revenir à maintenant'):t('Réinitialiser')}</button></div>
          <p className="text-[12px] text-muted mb-2">{phase==='live'?t('Faites glisser pour revoir un moment de la journée : puis revenez au direct.'):phase==='after'?"Rejouez la journée heure par heure sur le plan.":"Faites glisser le curseur pour voir comment se déroule une journée d'école, heure par heure (simple aperçu)."}</p>
