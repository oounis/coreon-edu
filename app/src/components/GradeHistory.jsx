          ...
          {hist.slice(0,12).map(ev=>{ const m=ev.mention||mentionFor(ev.score); return (
            <div key={ev.id} className="rounded-xl border border-line p-3">
              <div className="flex items-center justify-between mb-1">
                <div className="font-semibold text-sm inline-flex items-center gap-1.5">{ev.subject}{ev.lesson&&<span className="text-muted font-medium"> {ev.lesson}</span>} {ev.badge&&<span className="inline-flex items-center gap-1 accent-text">{ev.badge.icon&&<Ic n={ev.badge.icon} size={13}/>} {ev.badge.label}</span>}</div>
