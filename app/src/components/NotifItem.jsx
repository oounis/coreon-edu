...
export const KIND={
  request:{tint:'#36C5F0',Icon:FileText}, incident:{tint:'#FF6B81',Icon:ShieldAlert},
  payment:{tint:'#FFA62B',Icon:CreditCard}, notice:{tint:'#6C5CE7',Icon:Megaphone},
  message:{tint:'#2BD9A8',Icon:MessageSquare}, evaluation:{tint:'#8B5CF6',Icon:Star},
  info:{tint:'#8A93A6',Icon:CalendarCheck},
}
...
export function NotifRow({ n, onClick, compact }){
  ...
  const initials=n.actor? n.actor.split(' ').map(w=>w[0]).slice(0,2).join('') : null
  return (
    <button onClick={onClick} className={`w-full text-left flex items-start gap-3 ${compact?'px-2 py-2':'px-3 py-3'} rounded-2xl transition hover:bg-canvas ${!n.read?'bg-[color:var(--accent-soft)]':''}`}>
      <span className="relative shrink-0">
        <span className="w-11 h-11 rounded-full grid place-items-center text-white font-bold" style={{background:initials?k.tint:k.tint}}>{initials||<Icon size={18}/>}</span>
      ...
      <span className="flex-1 min-w-0">
        ...
        <span className="text-[11px] font-semibold mt-0.5 block" style={{color:!n.read?'var(--accent)':'#9aa3b5'}}>{ago(n.at)}</span>
      </span>
